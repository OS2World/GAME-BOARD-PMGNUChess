...
//              to convert the small amount of Windows code to OS/2 PM.
//
//  License:
//
...
#ifdef M_I386
extern struct hashentry ttable[2][ttblsz];
#else
extern struct hashentry far ttable[2][ttblsz];
#endif

#endif /* ttblsz */

extern unsigned char far history[8192];
...
{
  static short tempb, tempc, tempsf, tempst, xside, rpt;
...

{
  ...
  for (pnt = pbst = TrPnt[ply];
       ...
       pnt++)
    {
      ...
      while (!flag.timeout && WinPeekMsg(hab, &qmsg, NULL, 0, 0, PM_REMOVE))
...
/*      if (history[j] < 150)
      ...
      if (*(history+j) < 150)
        *(history+j) += (unsigned char) 2 * depth;
      if ((SHORT)node->t != (SHORT)(GameList[GameCnt].gmove & 0xFF))
        if (best <= beta)
          killr3[ply] = mv;
...

{
  ...
  for (i = 1; ptbl->hashbd != hashbd && i <= (USHORT)rehash; i++)
...

{
  ...
  /* rehash max rehash times */
  for (i = 1; depth < (SHORT)ptbl->depth && ptbl->hashbd != hashbd && i <= (USHORT)rehash; i++)
    ptbl = &ttable[side][(hashkey + i) & (ttblsz - 1)];
